...
function searchBooths() {
    try {
        ...
        if (input.trim()) {
            showToast(`📍 Showing booths near "${input}"`);
            // If Google map is initialized, we could try to geocode here
            if (window.map && window.google) {
                const geocoder = new google.maps.Geocoder();
                geocoder.geocode({ address: input + ', India' }, (results, status) => {
                    if (status === 'OK') {
...
document.addEventListener('DOMContentLoaded', () => {
    try {
        // Firebase setup (mock config)
        if (typeof firebase !== 'undefined') {
            const firebaseConfig = {
                apiKey: "mock-key",
                authDomain: "votewise-mock.firebaseapp.com",
                projectId: "votewise-mock",
                storageBucket: "votewise-mock.appspot.com",
