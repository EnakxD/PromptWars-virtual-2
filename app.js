...
function earnXP(amount, msg) {
    ...
    if (window.analytics) {
        window.analytics.logEvent('earn_xp', { amount: amount, new_level: state.level });
    }
    if (window.db) {
        saveUserData();
    }
...
function searchBooths() {
    try {
        const rawInput = document.getElementById('location-input').value;
        const input = sanitizeInput(rawInput);
        if (input.trim()) {
            showToast(`📍 Searching booths near "${input}"`);
            
            if (window.map && window.google) {
                const request = {
                    query: input + ' polling booth',
                ...
                service.findPlaceFromQuery(request, function(results, status) {
                    if (status === google.maps.places.PlacesServiceStatus.OK && results) {
                        ...
                        showToast('Could not find booths exactly there. Showing area.');
                        const geocoder = new google.maps.Geocoder();
                        geocoder.geocode({ address: input + ', India' }, (geoResults, geoStatus) => {
                            if (geoStatus === 'OK') {
                                window.map.setCenter(geoResults[0].geometry.location);
                ...
                // Fallback to iframe if API is missing or fails
                const mapArea = document.getElementById('google-map-area');
                const fakeMap = document.getElementById('map-area');
                if (mapArea && fakeMap) {
                    fakeMap.style.display = 'none';
                    mapArea.style.display = 'block';
                    mapArea.innerHTML = `<iframe width="100%" height="300" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://maps.google.com/maps?q=${encodeURIComponent(input + ', India')}&t=&z=14&ie=UTF8&iwloc=&output=embed"></iframe>`;
                }
            }
        } else {
        ...
        console.error("Search booths error:", err);
    }
}
                ...
                (position) => {
                    ...
                    if (window.map && window.google) {
                        ...
                        if (mapArea && fakeMap) {
                            fakeMap.style.display = 'none';
                            mapArea.style.display = 'block';
                            mapArea.innerHTML = `<iframe width="100%" height="300" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://maps.google.com/maps?q=${lat},${lng}&t=&z=14&ie=UTF8&iwloc=&output=embed"></iframe>`;
                        }
                    }
                },
...
function renderQuiz() {
    ...
    `;
        ...
        if (window.analytics) {
            window.analytics.logEvent('quiz_completed', { score: state.quizScore, total: QUIZ_Q.length });
        }
        if (window.db) {
            saveUserData();
        }
...
function initMap() {
    try {
        const mapArea = document.getElementById('google-map-area');
        const fakeMap = document.getElementById('map-area');
        if (mapArea && window.google) {
            fakeMap.style.display = 'none';
            mapArea.style.display = 'block';
            
            const indiaCenter = { lat: 20.5937, lng: 78.9629 };
            window.map = new google.maps.Map(mapArea, {
                zoom: 5,
                center: indiaCenter,
            });
 ...
 */
function saveUserData(explicitUid = null) {
    if (!window.db) return;
    try {
        let userId = explicitUid;
        if (!userId) {
            userId = localStorage.getItem('votewise_user_id') || 'user_' + Date.now();
            localStorage.setItem('votewise_user_id', userId);
        }
...
/**
 * Setup Firebase and Service Worker
 */
document.addEventListener('DOMContentLoaded', () => {
    try {
        if (typeof firebase !== 'undefined') {
            const firebaseConfig = {
                apiKey: "AIzaSyB8mR0eICHP2KAqual9HbTOrq0TWaXkNKc",
                authDomain: "promptwars-2-5f208.firebaseapp.com",
                projectId: "promptwars-2-5f208",
                storageBucket: "promptwars-2-5f208.firebasestorage.app",
                messagingSenderId: "952429342376",
                appId: "1:952429342376:web:7035a9c3b1b0cf33f09c9d",
                measurementId: "G-J46NQXLJDX"
            };
            
            // Only initialize if the user replaced the placeholder
            if (firebaseConfig.apiKey !== "YOUR_FIREBASE_API_KEY") {
                firebase.initializeApp(firebaseConfig);
                if (typeof firebase.analytics === 'function') window.analytics = firebase.analytics();
                if (typeof firebase.firestore === 'function') window.db = firebase.firestore();
            } else {
                console.warn("Firebase not initialized. Replace placeholders in app.js.");
            }
        }
    } catch(e) {
    ...
    // Service Worker for Caching / Efficiency
